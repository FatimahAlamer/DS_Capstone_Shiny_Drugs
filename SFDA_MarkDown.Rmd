...
HumanDrugs[HumanDrugs=="montelukast sodium"] <- "montelukast"
HumanDrugs[HumanDrugs=="quetiapine fumarate"] <- "quetiapine"
HumanDrugs[HumanDrugs=="sildenafil citrate"] <- "sildenafil"
...
HumanDrugs  %>% filter(`Public price`< 1332,`Distribute area`!="NA" ) %>% 
  ggplot( aes(x = RegisterYear, y = `Public price`, color=`Legal Status`)) + 
 geom_jitter(alpha = 0.7)+
  ...
  facet_grid(~Drug_Status)+
      theme_minimal()+
   labs(
       title = "Public Price Vs Registration Year")+
...
theme_minimal()+
   labs(
       title = "Most Common Used Route of Administration")
```
finding our focus
...
sdrugs%>% filter ( AdministrationRoute =="Oral") %>% group_by (`Scientific Name`) %>% 
  summarise(`Trade Name` = n()) %>% arrange(desc(`Trade Name`))%>% head(10) %>% ggplot( aes(x=fct_reorder(`Scientific Name`,`Trade Name`), y=`Trade Name`, fill=`Scientific Name`)) +
  geom_bar(stat="identity")+
  theme_minimal()+
    scale_fill_brewer(palette="Set3")+
  labs(x = "Scientific Name",
       y = "Number of Trade name",
       title = "Top 10 oral drugs manufactured by different companies in Saudi Arabia")+
 coord_flip()
```
...
nosdrugs%>% filter ( AdministrationRoute =="Oral") %>% group_by (`Scientific Name`) %>% 
  ...
  labs(x = "Scientific Name",
       y = "Number of Trade name",
       title = "Top 10 oral drugs imported by different companies to Saudi Arabia")+
  coord_flip()
...
```
it seems that all of suspended drugs might have manufacture issues or any other issues rather than a problem in a drug itself, since all drugs are still in use
